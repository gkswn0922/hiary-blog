...
const loadTilPosts = async () => {
  ...
  try {
    ...
    // const response = await fetch(`${API_URL}/til`)
    ...
    const data = [
      ...
      },
      {
        ...
        description: "고차원 데이터 시각화를 위한 t-SNE의 원리를 이해하고 구현했습니다. PCA와의 차이점을 비교하며, 각 차원 축소 기법의 장단점과 적절한 사용 사례를 학습했습니다.",
        tags: "t-SNE, PCA, dimension",
        date: "2025년 8월 29일"
      },
      {
        id: 4,
        title: "Optimizer 시리즈 심층 분석 - SGD부터 AdamW까지",
        description: "각 옵티마이저의 동작 원리와 수학적 배경을 이해했습니다. Momentum, RMSProp, Adam 등의 발전 과정을 따라가며 per-parameter 학습률 조정의 중요성을 깨달았고, 실제 프로젝트에서의 선택 기준을 정리했습니다.",
        tags: "Optimization, SGD, AdamW",
        date: "2025년 8월 28일"
      },
      {
        id: 5,
      ...
      {
        id: 6,
        title: "배치 정규화 vs 레이어 정규화 - 언제 무엇을 써야 할까?",
        description: "Batch Norm, Layer Norm, Instance Norm, Group Norm의 차이점을 실험으로 확인했습니다. 각 정규화 기법의 수학적 근거와 구현 방식을 비교하고, CNN과 RNN에서의 적절한 사용 사례를 정리했습니다.",
        tags: "Batch Norm, Layer Norm, Instance Norm",
        date: "2025년 8월 26일"
      },
      {
        id: 7,
        title: "ResNet과 DenseNet의 구조적 차이점 분석",
        description: "Skip connection의 개념부터 시작하여 ResNet과 DenseNet의 차이를 실제 구현으로 비교했습니다. Feature map의 재사용 방식과 gradient flow의 차이점을 분석하고, 각 모델의 장단점을 정리했습니다.",
        tags: "ResNet, DenseNet, Skip Connection",
        date: "2025년 8월 25일"
      },
      {
        id: 8,
        title: "GAN 구현하기 - Generator와 Discriminator의 균형",
        description: "DCGAN을 처음부터 구현하면서 생성자와 판별자의 학습 균형을 맞추는 방법을 학습했습니다. Mode collapse 문제와 해결 방안, 안정적인 학습을 위한 하이퍼파라미터 튜닝 경험을 정리했습니다.",
        tags: "GAN, DCGAN, Mode Collapse",
        date: "2025년 8월 24일"
      }
    ]

    if (data && Array.isArray(data)) {
      tilPosts.value = data.map(post => ({
